...
class HexBlock(tuple):
    ...
    def is_valid(self):
        ...
        return all((
            ...
            len(self) in config.NUMERO_MAX_BOLAS_ATAQUE,
...
class HexQuerySet(dict):
    ...
    def check_win(self, state):
        ...
        """
        if len(self.marbles(not state)) <= 2:
            return True
        return False
...
class BaseGrid(dict):
    ...
    def deep_copy(self, raw=False):
        copy = {
            config.BRANCO: [(k[0], k[1]) for k, v in self.items() if v == config.BRANCO],
            config.PRETO: [(k[0], k[1]) for k, v in self.items() if v == config.PRETO],
        }
        if raw == True:
            return copy
    ...
    def moves(self, state, rnd=False, seed=None):
        ...
        """
        lengths = config.NUMERO_MAX_BOLAS_ATAQUE
        if rnd:
...
class AbaloneGrid(BaseGrid):
    def __init__(self, initial_position):
        super(AbaloneGrid, self).__init__(config.TAMANHO_TABULEIRO)
